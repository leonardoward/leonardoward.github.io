...
var c_level2= {
    ...
    create: function() {
        ...
        c2_bullets = game.add.sprite(10000, 10000, 'bullet');
        // Enable the physics engine
        game.physics.enable([c2_bullets], Phaser.Physics.ARCADE);
        // Configure physics behaviour
        c2_bullets.anchor.setTo(0.5, 0.5);
        c2_bullets.body.collideWorldBounds = true;
        c2_bullets.body.bounce.set(0.9);
        c2_bullets.body.drag.set(20, 20);
        ...
        c2_ball[0] = game.add.sprite(850, 300, 'ball');
        c2_ball_mass[0] = 100;

        c2_ball[1] = game.add.sprite(400, 300, 'ball');
        c2_ball_mass[1] = 100;


        // Iteratively create the obstacles
        for (var i = 0; i < c2_bar_numbers + c2_barn_numbers; i++)
        {   // POSITIVE OBSTACLES
            // if (i < c2_bar_numbers) {
            //     c2_ball[i] = game.add.sprite(game.world.randomX + 20, game.world.randomY + 20, 'ball');
            //     // Set a mass for each ball
            //     c2_ball_mass[i] = getRandomIntInclusive(20,50);
            // }
            // // NEGATIVE OBSTACLES
            // else {
            //     c2_ball[i] = game.add.sprite(game.world.randomX + 20, game.world.randomY + 20, 'ball2');
            //     //Set a mass for each ball
            //     c2_ball_mass[i] = (-1)*getRandomIntInclusive(20,50);
            // }

            //Make the balls collide with the screen edge.
            // c2_ball[i].body.collideWorldBounds = true;
            c2_ball[i].anchor.setTo(0.5, 0.5);
            var scale = 1;
            c2_ball[i].scale.setTo(1 + scale, 1 + scale);

            // Create group for the range indicators
            c2_ball_indicator_group[i] = game.add.group();
            c2_ball_indicator_group[i].x = c2_ball[i].x;
            c2_ball_indicator_group[i].y = c2_ball[i].y;
            var indicator_quantity = 50
            var indicator_base_range = 200;//*(1 - c2_ball[i].scale.x);
            for (var j = 0; j < indicator_quantity; j++) {
                c2_ball_indicator_group[i].create(indicator_base_range*Math.cos(2*3.14159265/indicator_quantity*j), indicator_base_range*Math.sin(2*3.14159265/indicator_quantity*j), 'range_indicator');
            }

            game.physics.enable(c2_ball, Phaser.Physics.ARCADE);
        }
        ...
        //################################# Texto ##############################################
        // c2_scoreText = game.add.text(16, 16, 'score: 0', { fontSize: '16px', fill: '#000' });

        //################################# Win Rectangle ##############################################
        // If the c2_bullets hits this, you win the game
        ...
        level_text = game.add.text(game.width / 2, 80, 'Nivel 2', {font: '20px Arial', fill: '#ffffff'});
        ...
        tut_text5 = game.add.text(500, 300, '     Los obstáculos en el camino\nSacaran a la particula de su curso\n', {font: '20px Arial', fill: '#ffffff'});
        // Science instructions
        tut_pic1 = game.add.sprite(500, 400, "clvl2_tut1");
     ...
     update: function() {
        ...
        if (game_paused){
            ...
            for (var i = 0; i < c2_ball_indicator_group.length; i++) c2_ball_indicator_group[i].rotation += 0.01;

            //################################# Frame per frame calculations  ##############################################
            //Kill particle on impact
            game.physics.arcade.overlap(c2_bullets, c2_ball, this.particleImpact, null, this);
            arrow.rotation = game.physics.arcade.angleBetween(c2_bullets, arrow);
            ...
            {
                c2_bullets.x = game.input.activePointer.worldX;
                c2_bullets.y = game.input.activePointer.worldY;

                set(c2_bullets,game.input.activePointer);
            ...
            if (catchFlag == true){
                ...
                analog.height = game.physics.arcade.distanceBetween(arrow, c2_bullets);
                launchVelocity = analog.height;
            }
            // Release the mouse to shoot the particle.
            if (game.input.activePointer.isUp && catchFlag == true) this.c2_launch();

            //################################# Update the Trayectory trail  ##############################################
            // Only draw the trail if the bullet is on screen and the bullet is not on the slingshot
            if (c2_bullets.x < 5000 && catchFlag == false ){
                // Draw a small rectangle en the position of the bullet
                bmd.context.fillRect(c2_bullets.x, c2_bullets.y, 2, 2);
                // Persistence of the trail
                bmd.dirty = true;
            }
            //################################# Calculate the gavity over the bullet  ##############################################
            //Coulomb force calculation.
            c2_ForceCalculation(c2_bullets);

            //################################# Win Gate ##############################################
            // If the c2_bullets hits this, you win the game
            game.physics.arcade.overlap(c2_bullets, win_gate, this.win, null, this);
            win_gate.animations.play('run');


            //################################# Keyboard Controls ##############################################
            // If you hit R, reset the game.
            if (this.keyboard.isDown(Phaser.Keyboard.R)) game.state.start('c_level2');
            ...
            if (this.keyboard.isDown(Phaser.Keyboard.V)) c2_q_change();
        }
    ...
    c2_launch: function() {
        ...
        catchFlag = false;
        c2_bullets.body.moves = true;

        arrow.alpha = 0;
        analog.alpha = 0;

        Xvector = (arrow.x - c2_bullets.x) * 2.5;
        Yvector = (arrow.y - c2_bullets.y) * 2.5;

        c2_bullets.body.velocity.setTo(Xvector, Yvector);
    },
    ...
    particleImpact: function(bullet, ball){
        ...
        if (c2_bullet_color == 0){
            this.blue_emitter.x = c2_bullets.x;
            this.blue_emitter.y = c2_bullets.y;
            this.blue_emitter.start(true, 800, null, 20);
        }
        if (c2_bullet_color == 1){
            this.red_emitter.x = c2_bullets.x;
            this.red_emitter.y = c2_bullets.y;
            this.red_emitter.start(true, 800, null, 20);
        }
        // Removes the bullet from the screen
        bullet.x = 10000;
        bullet.y = 10000;
        c2_bullets.body.moves = false;
    }
...
function c2_ForceCalculation(child) {

    //Tuning variables
    var c2_G = 6;
    var c2_Radius_of_effect = 200;
    var c2_min_Radius_of_effect = 50;

    var c2_Accumulated_Gravity = new Phaser.Point(0,0);
    ...
    for (var i = 0; i < c2_bar_numbers + c2_barn_numbers; i++) {
        ...
        c2_Gravity_vector = new Phaser.Point(c2_ball[i].x - child.body.x, c2_ball[i].y - child.body.y);
        c2_Gravity_vector_test = new Phaser.Point(c2_ball[i].x - child.body.x, c2_ball[i].y - child.body.y);
        ...
        // var c2_Gravity_distance = floatConstraint(c2_Gravity_vector.getMagnitude() - 20*c2_ball[i].scale.x, c2_min_Radius_of_effect, Radius_of_effect);
        var c2_Gravity_distance = floatConstraint(c2_Gravity_vector.getMagnitude() - 100, c2_min_Radius_of_effect, c2_Radius_of_effect);

        // if (min_distance > c2_Gravity_vector.getMagnitude()){ min_distance = c2_Gravity_vector.getMagnitude(); min_distance_scale = c2_ball[i].scale.x}

        // Now, we calculate the actual magnitude of the gravitational force.
        c2_Gravity_Magnitude = c2_G*c2_bullet_mass*c2_ball_mass[i]/c2_Gravity_distance*c2_Gravity_distance;
        // Normalize the gravity vector and scale by the Gravity magnitude
        c2_Gravity_vector_scaled = c2_Gravity_vector.normalize().multiply(c2_Gravity_Magnitude,c2_Gravity_Magnitude);

        // Only let it act if the bullet is close enough
        if (c2_Gravity_vector_test.getMagnitude() < c2_Radius_of_effect)  c2_Accumulated_Gravity = Phaser.Point.add(c2_Accumulated_Gravity,c2_Gravity_vector_scaled);

    }
    child.body.gravity = c2_Accumulated_Gravity;
    // c2_scoreText.text = child.body.gravity;
    // c2_scoreText.text = win_gate.angle;
    // c2_scoreText.text = min_distance + '\n' + min_distance_scale;
}

// // Change the sprite and sign of the charge of every obstacle.
// function c2_q_change() {
//
...
//
//         for (var i = 0; i < c2_bar_numbers + c2_barn_numbers; i++) c2_ball_mass[i] = (-1)*c2_ball_mass[i];
//         if (c2_bullet_color == 0) {c2_bullets.loadTexture('bullet2',0); c2_bullet_color = 1;}
//         else {c2_bullets.loadTexture('bullet',0); c2_bullet_color = 0;}
//     }
//
// }
...
function set(c2_bullets,pointer) {

    catchFlag = true;

    c2_bullets.body.moves = false;
    c2_bullets.body.velocity.setTo(0, 0);
    arrow.reset(c2_bullets.x, c2_bullets.y);
    analog.reset(c2_bullets.x, c2_bullets.y);
...
/* SC5
1) Added trayectory trail for c2_bullets
