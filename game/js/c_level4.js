...
var c_level4= {
    ...
    create: function() {
        ...
        c4_bullets = game.add.sprite(10000, 10000, 'bullet');
        // Enable the physics engine
        game.physics.enable([c4_bullets], Phaser.Physics.ARCADE);
        // Configure physics behaviour
        c4_bullets.anchor.setTo(0.5, 0.5);
        c4_bullets.body.collideWorldBounds = true;
        c4_bullets.body.bounce.set(0.9);
        c4_bullets.body.drag.set(20, 20);
        ...
        c4_ball[0] = game.add.sprite(800, 150, 'ball2');
        c4_ball_mass[0] = -400;

        c4_ball[1] = game.add.sprite(800, 450, 'ball2');
        c4_ball_mass[1] = -400;


        // Iteratively create the obstacles
        for (var i = 0; i < c4_bar_numbers + c4_barn_numbers; i++)
        {   // POSITIVE OBSTACLES
            // if (i < c4_bar_numbers) {
            //     c4_ball[i] = game.add.sprite(game.world.randomX + 20, game.world.randomY + 20, 'ball');
            //     // Set a mass for each ball
            //     c4_ball_mass[i] = getRandomIntInclusive(20,50);
            // }
            // // NEGATIVE OBSTACLES
            // else {
            //     c4_ball[i] = game.add.sprite(game.world.randomX + 20, game.world.randomY + 20, 'ball2');
            //     //Set a mass for each ball
            //     c4_ball_mass[i] = (-1)*getRandomIntInclusive(20,50);
            // }

            //Make the balls collide with the screen edge.
            // c4_ball[i].body.collideWorldBounds = true;
            c4_ball[i].anchor.setTo(0.5, 0.5);
            var scale = 1.5;
            c4_ball[i].scale.setTo(1 + scale, 1 + scale);

            // Create group for the range indicators
            c4_ball_indicator_group[i] = game.add.group();
            c4_ball_indicator_group[i].x = c4_ball[i].x;
            c4_ball_indicator_group[i].y = c4_ball[i].y;
            var indicator_quantity = 50
            var indicator_base_range = 250;//*(1 - c4_ball[i].scale.x);
            for (var j = 0; j < indicator_quantity; j++) {
                c4_ball_indicator_group[i].create(indicator_base_range*Math.cos(2*3.14159265/indicator_quantity*j), indicator_base_range*Math.sin(2*3.14159265/indicator_quantity*j), 'range_indicator');
            }

            game.physics.enable(c4_ball, Phaser.Physics.ARCADE);
        }
        ...
        //################################# Texto ##############################################
        // c4_scoreText = game.add.text(16, 16, 'score: 0', { fontSize: '16px', fill: '#000' });

        //################################# Win Rectangle ##############################################
        // If the c4_bullets hits this, you win the game
        ...
        //################################# Pretty Win gate  ##############################################
        win_gate = game.add.sprite(c4_win_x, c4_win_y, 'win_gate');
        ...
        level_text = game.add.text(game.width / 2, 80, 'Nivel 4', {font: '20px Arial', fill: '#ffffff'});
        ...
        // Play Button
        tut_play_button = game.add.button(1100, 20, "play_button2",c4_toggleMenu);
        ...
        tut_text5 = game.add.text(450, 420, 'El espacio entre 2 obstaculos es más seguro\nPorque te jalan de igual de ambos lados\nY sales en la misma dirección, más rápido', {font: '20px Arial', fill: '#ffffff'});
        // Science instructions
        tut_pic1 = game.add.sprite(430, 150, "clvl4_tut1");
     ...
     update: function() {
        ...
        if (game_paused){
            ...
            if (this.keyboard.isDown(Phaser.Keyboard.I)) c4_toggleMenu();
        }
        else{
            //################################# Animate the Range Indicators  ##############################################
            for (var i = 0; i < c4_ball_indicator_group.length; i++) c4_ball_indicator_group[i].rotation += 0.01;

            //################################# Frame per frame calculations  ##############################################
            //Kill particle on impact
            game.physics.arcade.overlap(c4_bullets, c4_ball, this.particleImpact, null, this);
            arrow.rotation = game.physics.arcade.angleBetween(c4_bullets, arrow);
            ...
            {
                c4_bullets.x = game.input.activePointer.worldX;
                c4_bullets.y = game.input.activePointer.worldY;

                set(c4_bullets,game.input.activePointer);
            ...
            if (catchFlag == true){
                ...
                analog.height = game.physics.arcade.distanceBetween(arrow, c4_bullets);
                launchVelocity = analog.height;
            }
            // Release the mouse to shoot the particle.
            if (game.input.activePointer.isUp && catchFlag == true) this.c4_launch();

            //################################# Update the Trayectory trail  ##############################################
            // Only draw the trail if the bullet is on screen and the bullet is not on the slingshot
            if (c4_bullets.x < 5000 && catchFlag == false ){
                // Draw a small rectangle en the position of the bullet
                bmd.context.fillRect(c4_bullets.x, c4_bullets.y, 2, 2);
                // Persistence of the trail
                bmd.dirty = true;
            }
            //################################# Calculate the gavity over the bullet  ##############################################
            //Coulomb force calculation.
            c4_ForceCalculation(c4_bullets);

            //################################# Win Gate ##############################################
            // If the c4_bullets hits this, you win the game
            game.physics.arcade.overlap(c4_bullets, win_gate, this.win, null, this);
            win_gate.animations.play('run');


            //################################# Keyboard Controls ##############################################
            // If you hit R, reset the game.
            if (this.keyboard.isDown(Phaser.Keyboard.R)) game.state.start('c_level4');
            // If you hit M, Toogle the menu.
            if (this.keyboard.isDown(Phaser.Keyboard.M)) c4_toggleInGameMenu();
            // If you hit I, Toogle the menu.
            if (this.keyboard.isDown(Phaser.Keyboard.I)) c4_toggleMenu();

            //################################# Charge inversion mechanic #############################################
            //Invert the sign of the masses
            if (this.keyboard.isDown(Phaser.Keyboard.V)) c4_q_change();
        }
    ...
    particleImpact: function(bullet, ball){
        ...
        if (c4_bullet_color == 0){
            this.blue_emitter.x = c4_bullets.x;
            this.blue_emitter.y = c4_bullets.y;
            this.blue_emitter.start(true, 800, null, 20);
        }
        if (c4_bullet_color == 1){
            this.red_emitter.x = c4_bullets.x;
            this.red_emitter.y = c4_bullets.y;
            this.red_emitter.start(true, 800, null, 20);
        }
        // Removes the bullet from the screen
        bullet.x = 10000;
        bullet.y = 10000;
        c4_bullets.body.moves = false;
    },
    ...
    c4_launch: function() {
        ...
        catchFlag = false;
        c4_bullets.body.moves = true;

        arrow.alpha = 0;
        analog.alpha = 0;

        Xvector = (arrow.x - c4_bullets.x) * 2.5;
        Yvector = (arrow.y - c4_bullets.y) * 2.5;

        c4_bullets.body.velocity.setTo(Xvector, Yvector);
    }
...
function c4_ForceCalculation(child) {

    //Tuning variables
    var c4_G = 6;
    var c4_Radius_of_effect = 250;
    var c4_min_Radius_of_effect = 50;

    var c4_Accumulated_Gravity = new Phaser.Point(0,0);
    ...
    for (var i = 0; i < c4_bar_numbers + c4_barn_numbers; i++) {
        ...
        c4_Gravity_vector = new Phaser.Point(c4_ball[i].x - child.body.x, c4_ball[i].y - child.body.y);
        c4_Gravity_vector_test = new Phaser.Point(c4_ball[i].x - child.body.x, c4_ball[i].y - child.body.y);
        ...
        // var c4_Gravity_distance = floatConstraint(c4_Gravity_vector.getMagnitude() - 20*c4_ball[i].scale.x, c4_min_Radius_of_effect, Radius_of_effect);
        var c4_Gravity_distance = floatConstraint(c4_Gravity_vector.getMagnitude() - 100, c4_min_Radius_of_effect, c4_Radius_of_effect);

        // if (min_distance > c4_Gravity_vector.getMagnitude()){ min_distance = c4_Gravity_vector.getMagnitude(); min_distance_scale = c4_ball[i].scale.x}

        // Now, we calculate the actual magnitude of the gravitational force.
        c4_Gravity_Magnitude = c4_G*c4_bullet_mass*c4_ball_mass[i]/c4_Gravity_distance*c4_Gravity_distance;
        // Normalize the gravity vector and scale by the Gravity magnitude
        c4_Gravity_vector_scaled = c4_Gravity_vector.normalize().multiply(c4_Gravity_Magnitude,c4_Gravity_Magnitude);

        // Only let it act if the bullet is close enough
        if (c4_Gravity_vector_test.getMagnitude() < c4_Radius_of_effect)  c4_Accumulated_Gravity = Phaser.Point.add(c4_Accumulated_Gravity,c4_Gravity_vector_scaled);

    }
    child.body.gravity = c4_Accumulated_Gravity;
    // c4_scoreText.text = child.body.gravity;
    // c4_scoreText.text = win_gate.angle;
    // c4_scoreText.text = min_distance + '\n' + min_distance_scale;
}

// Change the sprite and sign of the charge of every obstacle.
function c4_q_change() {
    ...
    if (game.time.now > q_next_change )
    {
        ...
        // for (var i = 0; i < c4_bar_numbers + c4_barn_numbers; i++) c4_ball_mass[i] = (-1)*c4_ball_mass[i];
        c4_bullet_mass = (-1)*c4_bullet_mass;
        if (c4_bullet_color == 0) {c4_bullets.loadTexture('bullet2',0); c4_bullet_color = 1;}
        else {c4_bullets.loadTexture('bullet',0); c4_bullet_color = 0;}
    }
...
function set(c4_bullets,pointer) {

    catchFlag = true;

    c4_bullets.body.moves = false;
    c4_bullets.body.velocity.setTo(0, 0);
    arrow.reset(c4_bullets.x, c4_bullets.y);
    analog.reset(c4_bullets.x, c4_bullets.y);
...
/* SC5
1) Added trayectory trail for c4_bullets
