...
var c6_game_already_started = false;
...
var c_level6= {
    ...
    create: function() {
        ...
        c6_bullets = game.add.sprite(10000, 10000, 'bullet');
        // Enable the physics engine
        game.physics.enable([c6_bullets], Phaser.Physics.ARCADE);
        // Configure physics behaviour
        c6_bullets.anchor.setTo(0.5, 0.5);
        c6_bullets.body.collideWorldBounds = true;
        c6_bullets.body.bounce.set(0.9);
        c6_bullets.body.drag.set(20, 20);
        ...
        c6_bar_numbers = getRandomIntInclusive(1,3);
        c6_barn_numbers = getRandomIntInclusive(1,3);

        // Random win gate
         c6_win_x = getRandomIntInclusive(600,1200);
         c6_win_y = getRandomIntInclusive(0,600);

        // Iteratively create the obstacles
        for (var i = 0; i < c6_bar_numbers + c6_barn_numbers; i++)
        {   // POSITIVE OBSTACLES
            if (i < c6_bar_numbers) {
                c6_ball[i] = game.add.sprite(getRandomIntInclusive(270,1200), getRandomIntInclusive(0,600), 'ball');
                // Set a mass for each ball
                c6_ball_mass[i] = getRandomIntInclusive(50,100);
            }
            // NEGATIVE OBSTACLES
            else {
                c6_ball[i] = game.add.sprite(game.world.randomX + 1200/5, game.world.randomY + 20, 'ball2');
                //Set a mass for each ball
                c6_ball_mass[i] = (-1)*getRandomIntInclusive(50,100);
            }

            //Make the balls collide with the screen edge.
            // c6_ball[i].body.collideWorldBounds = true;
            c6_ball[i].anchor.setTo(0.5, 0.5);
            var scale = Math.random()*1.5;
            c6_ball[i].scale.setTo(1 + scale, 1 + scale);

            // Create group for the range indicators
            c6_ball_indicator_group[i] = game.add.group();
            c6_ball_indicator_group[i].x = c6_ball[i].x;
            c6_ball_indicator_group[i].y = c6_ball[i].y;
            var indicator_quantity = 50
            var indicator_base_range = 200;//*(1 - c6_ball[i].scale.x);
            for (var j = 0; j < indicator_quantity; j++) {
                c6_ball_indicator_group[i].create(indicator_base_range*Math.cos(2*3.14159265/indicator_quantity*j), indicator_base_range*Math.sin(2*3.14159265/indicator_quantity*j), 'range_indicator');
            }

            game.physics.enable(c6_ball, Phaser.Physics.ARCADE);
        }
        ...
        //################################# Texto ##############################################
        // c6_scoreText = game.add.text(16, 16, 'score: 0', { fontSize: '16px', fill: '#000' });

        //################################# Win Rectangle ##############################################
        // If the c6_bullets hits this, you win the game
        ...
        //################################# Pretty Win gate  ##############################################
        win_gate = game.add.sprite(c6_win_x, c6_win_y, 'win_gate');
        ...
        // Play Button
        tut_play_button = game.add.button(1100, 20, "play_button2",c6_toggleMenu);
        ...
        tut_text5 = game.add.text(430, 350, 'Presionar "R" para generar otro nivel aleatorio\nSi el generado es muy difícil\nNo todos se pueden ganar.', {font: '20px Arial', fill: '#ffffff'});
        // Science instructions
        tut_pic1 = game.add.sprite(580, 200, "clvl6_tut1");
        ...
        if (c6_game_already_started == false) game.add.tween(tut_menuGroup).to({ x: 0}, 1000, Phaser.Easing.Linear.Out, true);
        else game_paused = false;
     ...
     update: function() {
        ...
        if (game_paused){
            ...
            if (this.keyboard.isDown(Phaser.Keyboard.I)) c6_toggleMenu();
        }
        else{
            //################################# Animate the Range Indicators  ##############################################
            for (var i = 0; i < c6_ball_indicator_group.length; i++) c6_ball_indicator_group[i].rotation += 0.01;

            //################################# Frame per frame calculations  ##############################################
            //Kill particle on impact
            game.physics.arcade.overlap(c6_bullets, c6_ball, this.particleImpact, null, this);
            arrow.rotation = game.physics.arcade.angleBetween(c6_bullets, arrow);
            ...
            {
                c6_bullets.x = game.input.activePointer.worldX;
                c6_bullets.y = game.input.activePointer.worldY;

                set(c6_bullets,game.input.activePointer);
            ...
            if (catchFlag == true){
                ...
                analog.height = game.physics.arcade.distanceBetween(arrow, c6_bullets);
                launchVelocity = analog.height;
            }
            // Release the mouse to shoot the particle.
            if (game.input.activePointer.isUp && catchFlag == true) this.c6_launch();

            //################################# Update the Trayectory trail  ##############################################
            // Only draw the trail if the bullet is on screen and the bullet is not on the slingshot
            if (c6_bullets.x < 5000 && catchFlag == false ){
                // Draw a small rectangle en the position of the bullet
                bmd.context.fillRect(c6_bullets.x, c6_bullets.y, 2, 2);
                // Persistence of the trail
                bmd.dirty = true;
            }
            //################################# Calculate the gavity over the bullet  ##############################################
            //Coulomb force calculation.
            c6_ForceCalculation(c6_bullets);

            //################################# Win Gate ##############################################
            // If the c6_bullets hits this, you win the game
            game.physics.arcade.overlap(c6_bullets, win_gate, this.win, null, this);
            win_gate.animations.play('run');


            //################################# Keyboard Controls ##############################################
            // If you hit R, reset the game.
            if (this.keyboard.isDown(Phaser.Keyboard.R)) {c6_game_already_started = true; game.state.start('c_level6');}
            // If you hit M, Toogle the menu.
            if (this.keyboard.isDown(Phaser.Keyboard.M)) c6_toggleInGameMenu();
            // If you hit I, Toogle the menu.
            if (this.keyboard.isDown(Phaser.Keyboard.I)) c6_toggleMenu();

            //################################# Charge inversion mechanic #############################################
            //Invert the sign of the masses
            if (this.keyboard.isDown(Phaser.Keyboard.V)) c6_q_change();
        }
    ...
    particleImpact: function(bullet, ball){
        ...
        if (c6_bullet_color == 0){
            this.blue_emitter.x = c6_bullets.x;
            this.blue_emitter.y = c6_bullets.y;
            this.blue_emitter.start(true, 800, null, 20);
        }
        if (c6_bullet_color == 1){
            this.red_emitter.x = c6_bullets.x;
            this.red_emitter.y = c6_bullets.y;
            this.red_emitter.start(true, 800, null, 20);
        }
        // Removes the bullet from the screen
        bullet.x = 10000;
        bullet.y = 10000;
        c6_bullets.body.moves = false;
    },
    ...
    c6_launch: function() {
        ...
        catchFlag = false;
        c6_bullets.body.moves = true;

        arrow.alpha = 0;
        analog.alpha = 0;

        Xvector = (arrow.x - c6_bullets.x) * 2.5;
        Yvector = (arrow.y - c6_bullets.y) * 2.5;

        c6_bullets.body.velocity.setTo(Xvector, Yvector);
    }
...
function c6_ForceCalculation(child) {

    //Tuning variables
    var c6_G = 6;
    var c6_Radius_of_effect = 200;
    var c6_min_Radius_of_effect = 50;

    var c6_Accumulated_Gravity = new Phaser.Point(0,0);
    ...
    for (var i = 0; i < c6_bar_numbers + c6_barn_numbers; i++) {
        ...
        c6_Gravity_vector = new Phaser.Point(c6_ball[i].x - child.body.x, c6_ball[i].y - child.body.y);
        c6_Gravity_vector_test = new Phaser.Point(c6_ball[i].x - child.body.x, c6_ball[i].y - child.body.y);
        ...
        // var c6_Gravity_distance = floatConstraint(c6_Gravity_vector.getMagnitude() - 20*c6_ball[i].scale.x, c6_min_Radius_of_effect, Radius_of_effect);
        var c6_Gravity_distance = floatConstraint(c6_Gravity_vector.getMagnitude() - 100, c6_min_Radius_of_effect, c6_Radius_of_effect);

        // if (min_distance > c6_Gravity_vector.getMagnitude()){ min_distance = c6_Gravity_vector.getMagnitude(); min_distance_scale = c6_ball[i].scale.x}

        // Now, we calculate the actual magnitude of the gravitational force.
        c6_Gravity_Magnitude = c6_G*c6_bullet_mass*c6_ball_mass[i]/c6_Gravity_distance*c6_Gravity_distance;
        // Normalize the gravity vector and scale by the Gravity magnitude
        c6_Gravity_vector_scaled = c6_Gravity_vector.normalize().multiply(c6_Gravity_Magnitude,c6_Gravity_Magnitude);

        // Only let it act if the bullet is close enough
        if (c6_Gravity_vector_test.getMagnitude() < c6_Radius_of_effect)  c6_Accumulated_Gravity = Phaser.Point.add(c6_Accumulated_Gravity,c6_Gravity_vector_scaled);

    }
    child.body.gravity = c6_Accumulated_Gravity;
    // c6_scoreText.text = child.body.gravity;
    // c6_scoreText.text = win_gate.angle;
    // c6_scoreText.text = min_distance + '\n' + min_distance_scale;
}

// Change the sprite and sign of the charge of every obstacle.
function c6_q_change() {
    ...
    if (game.time.now > q_next_change )
    {
        ...
        // for (var i = 0; i < c6_bar_numbers + c6_barn_numbers; i++) c6_ball_mass[i] = (-1)*c6_ball_mass[i];
        c6_bullet_mass = (-1)*c6_bullet_mass;
        if (c6_bullet_color == 0) {c6_bullets.loadTexture('bullet2',0); c6_bullet_color = 1;}
        else {c6_bullets.loadTexture('bullet',0); c6_bullet_color = 0;}
    }
...
function set(c6_bullets,pointer) {

    catchFlag = true;

    c6_bullets.body.moves = false;
    c6_bullets.body.velocity.setTo(0, 0);
    arrow.reset(c6_bullets.x, c6_bullets.y);
    analog.reset(c6_bullets.x, c6_bullets.y);
...
/* SC5
1) Added trayectory trail for c6_bullets
